...
//TENV checkExp(exp:natCon(Natural N), Ttype req, TENV env) =
//	return;
...
TENV checkExp(exp:id(Id Id), Ttype req, TENV env) { // <5>
  if(!env.symbols[Id]?)
...
TENV checkStat(stat:whileStat(Expression Exp, 
                             list[Statement] Stats1),
                 TENV env) {
    env0 = checkExp(Exp, natural(), env);
    env1 = checkStats(Stats1, env0);
...
TENV checkDecls(list[Declaration] Decls) = // <12>
