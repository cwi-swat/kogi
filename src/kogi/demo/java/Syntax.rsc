...
module kogi::demo::java::Syntax

import ParseTree;

start syntax CompilationUnit =
   compilationUnit: PackageDec? pack ImportDec* import TypeDec* typeDec
  ;
  
syntax PackageDec =
   packageDec: Anno* anno "package"  PackageName packageName ";" 
  ;
...
syntax ImportDec 
  = typeImportDec: "import"  TypeName tipo ";" 
  | typeImportOnDemandDec: "import"  PackageName packageName "." "*" ";" 
  | staticImportOnDemandDec: "import"  "static"  TypeName typoName"." "*" ";" 
  | staticImportDec: "import"  "static"  TypeName typeName "." Id id";" 
  ;
...
syntax InterfaceDec 
  = AnnoDecHead annoDecHead "{" AnnoElemDec* annoElemnDec "}"
  | InterfaceDecHead interfaceDecHead "{" InterfaceMemberDec* interfaceMemberDec "}" 
  ;

syntax AnnoDecHead 
  = annoDecHead: (InterfaceMod | Anno)* anno "@" "interface"  Id id 
  ;
...
syntax AnnoElemDec
  ...
  | interfaceDec: InterfaceDec 
  | annoMethodDec: (Anno anno| AbstractMethodMod abstractMethodMod)* Type tipo Id id "(" ")" DefaultVal? defaultVal ";" 
  ;
  
syntax InterfaceDecHead =
   interfaceDecHead: (InterfaceMod interfaceMod | Anno anno)* "interface"  Id id TypeParams? typeParams ExtendsInterfaces? extendsInterfaces 
  ;

syntax LocalVarDec =
  @prefer localVarDec: (Anno anno | VarMod varMod)* Type tipo {VarDec ","}+ varDecs 
  ;
...
syntax Expr =
  field: FieldAccess fieldAccess \ FieldAccessKeywords 
  ...
  | ArrayCreationExpr 
  |  this: "this"  
  | array: ArrayAccess arrayAccess \ ArrayAccessKeywords 
  ;
...
syntax MethodSpec =
  ...
  |  superMethod: "super" super "." dot TypeArgs? Id 
